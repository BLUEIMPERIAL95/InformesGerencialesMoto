...
Public Class database_informes
#Region "ATRIBUTOS"

    'VARIABLES
    Private _connstring As String = ConfigurationManager.ConnectionStrings("SystramInformesconnectionstringtamos").ConnectionString
...
#Region "PUBLICOS"
    ...
    Function exec_query(ByVal sql As String) As DataTable
        ...
        If ConfigurationManager.AppSettings("bdsel").ToString = 1 Then
            oconn = New MySqlConnection(_connstring)
        Else
            If ConfigurationManager.AppSettings("bdsel").ToString = 2 Then
                oconn = New MySqlConnection(_connstringtar)
            Else
                oconn = New MySqlConnection(_connstringrefri)
            End If
        End If
        Dim ocomm = New MySqlCommand(sql, oconn)
        Dim adap = New MySqlDataAdapter(ocomm)
