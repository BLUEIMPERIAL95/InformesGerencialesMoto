...
Imports System.util
Partial Class InfParamCostoFinancieroSucursal
    Inherits System.Web.UI.Page
    Dim csinformes As New Informes
    Dim csusua As New usuarios
    Dim csoper As New Operaciones

    Private Sub InfParamCostoFinancieroSucursal_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        strRespuestaPer = csusua.validar_permiso_usuario(3061, Session("id_usua"))
    ...
    Private Sub btnPrueba_Click(sender As Object, e As EventArgs) Handles btnPrueba.Click
        Try
            ...
            Else
                ...
                strSQL = " Select mtc.sucorigina_nombre As 'Cliente',"
                ...
                strSQL += " CASE WHEN mtc.sucorigina_nombre = 'PRINCIPAL' Or mtc.sucorigina_nombre = 'FUNZA' THEN ROUND(7 * 30 *"
                strSQL += " COALESCE((SELECT COALESCE(SUM(vcc.venta_total - vcc.venta_abonos), 0)"
                ...
                strSQL += " And vcc.sucursal_origina_id = ven.sistema_sucursales_origina), 0) /"
                strSQL += " (SELECT SUM(vcc.venta_total)"
                strSQL += " From ventas_consolidado vcc"
                strSQL += " Where Year(vcc.venta_fecha) = Year(SYSDATE())"
                strSQL += " And vcc.sucursal_origina_id = ven.sistema_sucursales_origina), 0) ELSE 50 END AS DiasCartera,"
                strSQL += " CASE WHEN mtc.sucorigina_nombre = 'PRINCIPAL' Or mtc.sucorigina_nombre = 'FUNZA' THEN ROUND(((7 * 30 *"
                strSQL += " COALESCE((SELECT COALESCE(SUM(vcc.venta_total - vcc.venta_abonos), 0)"
                strSQL += " From ventas_consolidado vcc"
                strSQL += " Where vcc.venta_id = (Select vd.ventas_idventas From ventas_detalles vd Where vcc.venta_id = vd.ventas_idventas And vd.idel = 0 LIMIT 1)"
                ...
                strSQL += " From ventas_consolidado vcc"
                strSQL += " Where Year(vcc.venta_fecha) = Year(SYSDATE())"
                strSQL += " And vcc.sucursal_origina_id = ven.sistema_sucursales_origina)) / 30), 2) ELSE 1 END AS 'PartCos',"
                strSQL += " CASE WHEN mtc.sucorigina_nombre = 'PRINCIPAL' Or mtc.sucorigina_nombre = 'FUNZA' THEN ROUND(((7 * 30 *"
                strSQL += " COALESCE((SELECT COALESCE(SUM(vcc.venta_total - vcc.venta_abonos), 0)"
                strSQL += " From ventas_consolidado vcc"
                strSQL += " Where vcc.venta_id = (Select vd.ventas_idventas From ventas_detalles vd Where vcc.venta_id = vd.ventas_idventas And vd.idel = 0 LIMIT 1)"
                ...
                strSQL += " From ventas_consolidado vcc"
                strSQL += " Where Year(vcc.venta_fecha) = Year(SYSDATE())"
                strSQL += " And vcc.sucursal_origina_id = ven.sistema_sucursales_origina)) / 30) / 100 *"
                strSQL += " (COALESCE((SELECT COALESCE(SUM(vcc.venta_total - vcc.venta_abonos), 0)"
                strSQL += " From ventas_consolidado vcc"
                strSQL += " Where vcc.venta_id = (Select vd.ventas_idventas From ventas_detalles vd Where vcc.venta_id = vd.ventas_idventas And vd.idel = 0 LIMIT 1)"
                strSQL += " And ((vcc.venta_total > (SELECT SUM(vrd.valor)FROM ventas_recaudos_detalle vrd WHERE vrd.ventas_idventas=vcc.venta_id)) Or (SELECT SUM(vrd.valor) FROM ventas_recaudos_detalle vrd WHERE vrd.ventas_idventas=vcc.venta_id)Is NULL)"
                ...
                strSQL += " And vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10"
                strSQL += " And vcc.venta_vence<=(SELECT( ADDDATE(SYSDATE(),INTERVAL vcc.venta_plazo DAY)))"
                strSQL += " And vcc.sucursal_origina_id = ven.sistema_sucursales_origina), 0)), 0) ELSE SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa) * (1 / 100) END AS 'Costo',"
                strSQL += " CASE WHEN mtc.sucorigina_nombre = 'PRINCIPAL' Or mtc.sucorigina_nombre = 'FUNZA' THEN SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa - mtc.movimiento_fl_tercero - mtc.movimiento_cnx_tercero - mtc.movimiento_anexos) - "
                strSQL += " ROUND(((7 * 30 *"
                strSQL += " COALESCE((SELECT COALESCE(SUM(vcc.venta_total - vcc.venta_abonos), 0)"
                strSQL += " From ventas_consolidado vcc"
                strSQL += " Where vcc.venta_id = (Select vd.ventas_idventas From ventas_detalles vd Where vcc.venta_id = vd.ventas_idventas And vd.idel = 0 LIMIT 1)"
                ...
                strSQL += " And vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10"
                strSQL += " And vcc.venta_vence<=(SELECT( ADDDATE(SYSDATE(),INTERVAL vcc.venta_plazo DAY)))"
                strSQL += " And vcc.sucursal_origina_id = ven.sistema_sucursales_origina), 0)), 0) ELSE "
                strSQL += " ROUND(SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa - mtc.movimiento_fl_tercero - mtc.movimiento_cnx_tercero - mtc.movimiento_anexos) - (SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa) * (1 / 100)), 0) END AS IngReal,"
                strSQL += " CASE WHEN mtc.sucorigina_nombre = 'PRINCIPAL' Or mtc.sucorigina_nombre = 'FUNZA' THEN ROUND((SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa - mtc.movimiento_fl_tercero - mtc.movimiento_cnx_tercero - mtc.movimiento_anexos) -"
                strSQL += " ROUND(((7 * 30 *"
                strSQL += " COALESCE((SELECT COALESCE(SUM(vcc.venta_total - vcc.venta_abonos), 0)"
                strSQL += " From ventas_consolidado vcc"
                strSQL += " Where vcc.venta_id = (Select vd.ventas_idventas From ventas_detalles vd Where vcc.venta_id = vd.ventas_idventas And vd.idel = 0 LIMIT 1)"
                ...
                strSQL += " And vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10"
                strSQL += " And vcc.venta_vence<=(SELECT( ADDDATE(SYSDATE(),INTERVAL vcc.venta_plazo DAY))) "
                strSQL += " And vcc.sucursal_origina_id = ven.sistema_sucursales_origina), 0)), 0)) / "
                strSQL += " SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa), 2) * 100 ELSE (ROUND(SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa - mtc.movimiento_fl_tercero - mtc.movimiento_cnx_tercero - mtc.movimiento_anexos) - (SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa) * (1 / 100)), 0) / (SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa))) * 100 END AS Inter_Real "
                'strSQL += " ROUND((ROUND(SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa) - (SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa) * (1 / 100)) / , 0)) / SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa), 0) END AS Inter_Real"
                strSQL += " From movimientos_transportes_consolidado As mtc"
                strSQL += " Left Join ventas ven ON ven.idventas = mtc.factura_id AND (ven.sistema_sucursales_origina = 1 Or ven.sistema_sucursales_origina = 21) "
                strSQL += " WHERE mtc.factura_fecha BETWEEN '" & txtFechaInicio.Value & "' AND '" & txtFechaFin.Value & "' "
                strSQL += " Group BY mtc.sucorigina_nombre"
