...
Imports System.util
Partial Class InfParamCostoFinancieroAsesor
    Inherits System.Web.UI.Page
    Dim csinformes As New Informes
    Dim csusua As New usuarios
    Dim csoper As New Operaciones

    Private Sub InfParamCostoFinancieroAsesor_Unload(sender As Object, e As EventArgs) Handles Me.Unload
        ...
        strRespuestaPer = csusua.validar_permiso_usuario(3062, Session("id_usua"))
    ...
    Private Sub btnPrueba_Click(sender As Object, e As EventArgs) Handles btnPrueba.Click
        Try
            ...
            Else
                ...
                'strSQL = " Select ven.asesor_nombre As 'Cliente', "
                ...
                'strSQL += " ROUND(ROUND(8 * 30 * COALESCE((SELECT SUM(vcc.venta_total - vcc.venta_abonos) "
                'strSQL += " From ventas_consolidado vcc Where vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10 And vcc.venta_fecha < '2022-08-31' "
                'strSQL += " And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA'), 0), 0) / "
                'strSQL += " (SELECT SUM(vcc.venta_total) From ventas_consolidado vcc Where Year(vcc.venta_fecha) = Year(SYSDATE()) And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA'), 0) AS DiasCartera, "
                'strSQL += " ROUND((ROUND(8 * 30 * COALESCE((SELECT SUM(vcc.venta_total - vcc.venta_abonos) "
                'strSQL += " From ventas_consolidado vcc Where vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10 And vcc.venta_fecha < '2022-08-31' "
                'strSQL += " And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA'), 0), 0) / "
                'strSQL += " (SELECT SUM(vcc.venta_total) From ventas_consolidado vcc Where Year(vcc.venta_fecha) = Year(SYSDATE()) And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA')) / 30, 0)  AS 'PartCos', "
                'strSQL += " ROUND((ROUND((ROUND(8 * 30 * COALESCE((SELECT SUM(vcc.venta_total - vcc.venta_abonos) "
                'strSQL += " From ventas_consolidado vcc Where vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10 And vcc.venta_fecha < '2022-08-31' "
                'strSQL += " And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA'), 0), 0) / "
                'strSQL += " (SELECT SUM(vcc.venta_total) From ventas_consolidado vcc Where Year(vcc.venta_fecha) = Year(SYSDATE()) And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA')) / 30, 0) / 100) * "
                'strSQL += " (SELECT SUM(vcc.venta_total - vcc.venta_abonos) "
                'strSQL += " From ventas_consolidado vcc Where vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10 And vcc.venta_fecha < '2022-08-31' "
                'strSQL += " And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA'), 0) As 'Costo', "
                ...
                'strSQL += " (SELECT SUM(vcc.venta_total - vcc.venta_abonos) "
                'strSQL += " From ventas_consolidado vcc Where vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10 And vcc.venta_fecha < '2022-08-31' "
                'strSQL += " And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA'), 0) As IngReal, "
                'strSQL += " ROUND(((SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa - mtc.movimiento_fl_tercero - mtc.movimiento_cnx_tercero - mtc.movimiento_anexos) - "
                'strSQL += " ROUND((ROUND((ROUND(8 * 30 * COALESCE((SELECT SUM(vcc.venta_total - vcc.venta_abonos) "
                'strSQL += " From ventas_consolidado vcc Where vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10 And vcc.venta_fecha < '2022-08-31' "
                'strSQL += " And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA'), 0), 0) / "
                ...
                'strSQL += "(SELECT SUM(vcc.venta_total - vcc.venta_abonos) "
                'strSQL += " From ventas_consolidado vcc Where vcc.venta_abonos < vcc.venta_total And (vcc.venta_total - vcc.venta_abonos) > 10 And vcc.venta_fecha < '2022-08-31' "
                'strSQL += " And vcc.asesor_documento = ven.asesor_documento AND vcc.venta_estado <> 'ANULADA'), 0)) / SUM(mtc.movimiento_fl_empresa + mtc.movimiento_cnx_empresa)) * 100, 0) As 'Inter_Real' "
                'strSQL += " From movimientos_transportes_consolidado As mtc "
                'strSQL += " Left Join ventas_consolidado ven ON ven.venta_id = mtc.factura_id "
                'strSQL += " WHERE mtc.factura_fecha BETWEEN '2022-08-01' AND '2022-08-31' "
                'strSQL += " Group BY ven.asesor_nombre ORDER BY IngReal DESC "
                ...
                strSQL += " Group BY ven.asesor_nombre ORDER BY Ingreso DESC "
